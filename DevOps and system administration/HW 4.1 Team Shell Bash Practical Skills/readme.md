...
* нет закрывающей скобки в конструктиве условия while, должно быть ((1==1)) *искала информацию между разницей while ((1==1)) и [1==1] и нашла объяснение https://gist.github.com/Titiaiev/dcb7298389d1276b823bbc96e29f940d где говорится о
```
...
```
2. Добавляем в скрипт закрывающую скобку while ((1==1)) и условие else break. 
3. Если первая команда возвратит не ноль, что означает её неуспешное выполнение, условие окажется ложным и запишет результат в curl.log и не пойдет по ветке else, в противном случае, при возвращении 0 (успеха выполнения команды)  пойдет по ветке else, далее break (завершение скрипта - без записи в файл curl.log.)
4. выполняем скрипт и видим, что порт 4346 несуществует на моей машине, достучаться до него невозможно - поэтому идет ветка date >> curl.log
и записывает в файл и не идет по ветке else.
...
> т.к. изначально 192.168.0.1 недоступен, а остальные доступны - переместим 192.168.0.1 вконец проверки 
