...
перенаправила поток stderr в stdout
...
````
root@ubuntu2:/# echo ping 8.8.8.8 > lOGG
vagrant@ubuntu2:/$ ps -ef | grep 8.8.8.8
root     3223  2458  0 13:36 pts/0    00:00:00 ping 8.8.8.8
vagrant@ubuntu2:/$ sudo rm lOGG
vagrant@ubuntu2:/$ sudo lsof -p 3223
root@ubuntu2:/# lsof -p 3223
COMMAND  PID USER   FD   TYPE DEVICE SIZE/OFF    NODE NAME
ping    3223 root    1w   REG  253,0    41542    7002 /lOGG (deleted)

root@ubuntu2:/# cat /dev/null > /lOGG
root@ubuntu2:/# ls -l
...
-rw-r--r--   1 root root       0 Dec 23 13:51 lOGG
...
````

после команды ping можно посмотреть двумя способами:
через tail -f /proc/3223/fd/1 и через tail -f /lOGG

````
root@ubuntu2:/proc# tail -f /proc/3223/fd/1
64 bytes from 8.8.8.8: icmp_seq=64 ttl=109 time=30.4 ms
64 bytes from 8.8.8.8: icmp_seq=65 ttl=109 time=17.9 ms
64 bytes from 8.8.8.8: icmp_seq=66 ttl=109 time=38.8 ms
64 bytes from 8.8.8.8: icmp_seq=67 ttl=109 time=55.5 ms
64 bytes from 8.8.8.8: icmp_seq=68 ttl=109 time=17.7 ms
64 bytes from 8.8.8.8: icmp_seq=69 ttl=109 time=15.8 ms
64 bytes from 8.8.8.8: icmp_seq=70 ttl=109 time=16.6 ms

root@ubuntu2:/proc# tail -f /lOGG
64 bytes from 8.8.8.8: icmp_seq=196 ttl=109 time=17.9 ms
64 bytes from 8.8.8.8: icmp_seq=197 ttl=109 time=16.0 ms
64 bytes from 8.8.8.8: icmp_seq=198 ttl=109 time=16.6 ms
64 bytes from 8.8.8.8: icmp_seq=199 ttl=109 time=16.6 ms
64 bytes from 8.8.8.8: icmp_seq=200 ttl=109 time=16.2 ms
````

далее , если убить сам файл lOGG, то файл исчезнет, а процесс останется и наблюдать его можно через tail -f /proc/3223/fd/1

Обнулить файл можно пятью способами, которые все попробовала:

* echo '' > /proc/3223/fd/1
* :> /proc/3223/fd/1
* \> /proc/3223/fd/1
* truncate -s 0 /proc/3223/fd/1
* cat /dev/null > /proc/3223/fd/1

*во всех случаях, процесс не получается обнулить и зомби-файл продолжает свой рост*

````
root@ubuntu2:/# lsof -p 3223
COMMAND  PID USER   FD   TYPE DEVICE SIZE/OFF    NODE NAME
ping    3223 root  cwd    DIR  253,0     4096       2 /
ping    3223 root  rtd    DIR  253,0     4096       2 /
ping    3223 root  txt    REG  253,0    64424 3670086 /bin/ping
ping    3223 root  mem    REG  253,0  3004464 3937049 /usr/lib/locale/locale-archive
ping    3223 root  mem    REG  253,0  2030928 1049005 /lib/x86_64-linux-gnu/libc-2.27.so
ping    3223 root  mem    REG  253,0    97072 1049096 /lib/x86_64-linux-gnu/libresolv-2.27.so
ping    3223 root  mem    REG  253,0   219304 3934419 /usr/lib/x86_64-linux-gnu/libnettle.so.6.5
ping    3223 root  mem    REG  253,0   206872 1049042 /lib/x86_64-linux-gnu/libidn.so.11.6.16
ping    3223 root  mem    REG  253,0    22768 1049010 /lib/x86_64-linux-gnu/libcap.so.2.25
ping    3223 root  mem    REG  253,0   179152 1048981 /lib/x86_64-linux-gnu/ld-2.27.so
ping    3223 root    0u   CHR  136,0      0t0       3 /dev/pts/0
ping    3223 root    1w   REG  253,0    75382    7002 /lOGG (deleted)
````
...
![image.png](./assets/1640276586188-image.png)

К сожалению, не помогло и opensnoop-bpfcc все так-же не работает...
